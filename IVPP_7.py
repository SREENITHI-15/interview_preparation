...
def detect_shape(contour):
    ...
    # second parameter closed contour
    perimeter = cv2.arcLength(contour, True)

    # Get a contour approximation:
    # contour shape to another shape with less number of vertices depending upon the precision
...
for i, (size, contour) in enumerate(zip(contour_sizes, contours)):
    ...
    cv2.putText(image_recognition_shapes, str(i), (x+30, y+30), cv2.FONT_HERSHEY_SIMPLEX, 1., (0, 0, 0), 3)
    cv2.putText(image_recognition_shapes, str(size), (x+30, y+60), cv2.FONT_HERSHEY_SIMPLEX, 1., (0, 0, 0), 3)
